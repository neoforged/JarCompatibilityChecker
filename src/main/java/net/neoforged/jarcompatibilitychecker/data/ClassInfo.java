...
public class ClassInfo implements MemberInfo {
    ...
    public ClassInfo(ClassNode node) {
        ...
        if (!node.fields.isEmpty())
            this.fields = node.fields.stream().map(fieldNode -> new FieldInfo(fieldNode, this)).collect(Collectors.toMap(e -> e.name, e -> e));
        else
            this.fields = null;
    }
    ...
    public ClassInfo(Class<?> clazz) {
        ...
        if (flds.length > 0)
            this.fields = Arrays.stream(flds).map(field -> new FieldInfo(field, this)).collect(Collectors.toMap(e -> e.name, e -> e));
        else
            this.fields = null;
    }
