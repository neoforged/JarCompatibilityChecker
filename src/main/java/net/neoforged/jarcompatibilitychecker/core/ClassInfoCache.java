...
public class ClassInfoCache {
        ...
        try (ZipFile zip = new ZipFile(file)) {
            for (Enumeration<? extends ZipEntry> entries = zip.entries(); entries.hasMoreElements(); ) {
                ...
                if (!entry.getName().endsWith(".class") || entry.getName().startsWith("."))
                    continue;
                // Skip anonymous classes since they aren't public and their names may vary
                if (entry.getName().matches(ANONYMOUS_CLASS)) {
                    continue;
                }
    ...
    private static final String ANONYMOUS_CLASS = ".*\\$\\d+\\.class";
        ...
        try (Stream<Path> walker = Files.walk(folder)) {
            ...
            for (Path entryPath : iterable) {
                ...
                if (name == null || !name.endsWith(".class") || name.startsWith("."))
                    continue;
                // Skip anonymous classes since they aren't public and their names may vary
                if (name.matches(ANONYMOUS_CLASS)) {
                    continue;
                }
